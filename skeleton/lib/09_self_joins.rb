...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
  SELECT DISTINCT
  a.num, a.company, stopb.name, b.num, b.company
  FROM 
    routes a
  JOIN
    routes b ON (a.company = b.company AND a.num = b.num)
  ...
  -- JOIN 
  --   stops stope ON (e.stop_id = stope.id)
  -- JOIN
  --   stops stopf ON (f.stop_id = stopf.id) 
  WHERE 
    stopa.name = 'Craiglockhart'
  ...
  -- stopb.name IN
      ...
      --         stops stopb ON (b.stop_id = stopb.id)
      --       WHERE 
      --         stopa.name = 'Sighthill'
      --       ) sight 
            -- AND
            -- stopb.name NOT IN (
            --   'Craiglockhart', 'Sighthill'
            -- )
            -- ON craig.stopb.name = sight.stopb.name
